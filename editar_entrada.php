...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    header('Location: /bitacora.php?status=error&message=' . urlencode('ID de registro no válido'));
    exit;
}

$registroId = (int)$_GET['id'];
$pageTitle = "Editar Registro - Gtek Logistics";
...
$registro = $bitacoraModel->getRegistroById($registroId);

if (!$registro) {
    header('Location: /bitacora.php?status=error&message=' . urlencode('Registro no encontrado'));
    exit;
}

// Si es la primera carga, llenar el formulario con los datos existentes
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    $formData = [
        ...
        'descripcion_mercancia' => $registro['descripcion_mercancia'],
        ...
        'consignatario_email' => $registro['consignatario_email'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            // 1. Actualizar o crear Consignatario
            ...
            // 2. Actualizar o crear Remitente
            ...
            if ($updateResult) {
                header('Location: /bitacora.php?status=success&message=' . urlencode('Registro actualizado exitosamente.'));
                exit;
            } else {
                $errors[] = "Error al actualizar el registro. Por favor, intente de nuevo.";
            }

        } catch (Exception $e) {
            error_log("Error al actualizar registro de entrada: " . $e->getMessage());
...
<div class="page-content">
    <div class="form-container">
        <h2>Editar Registro de Entrada</h2>
        <div class="entry-info">
            <strong>ID:</strong> <?php echo htmlspecialchars($registroId); ?> | 
            <strong>Registrado:</strong> <?php echo htmlspecialchars(date('d/m/Y H:i A', strtotime($registro['fecha_creacion']))); ?> | 
            <strong>BL:</strong> <?php echo htmlspecialchars($registro['num_conocimiento_embarque']); ?>
        </div>
        ...
        <form action="" method="POST" class="entry-form">
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <span class="read-only-field"><?php echo htmlspecialchars(date('d/m/Y H:i A', strtotime($registro['fecha_creacion']))); ?></span>
                </div>
                <div class="form-group">
